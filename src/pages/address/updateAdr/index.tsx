...
const UpdateAdr = () => {
  ...
  const [selectedWard, setSelecteWard] = useState(item.ward_id);
  const [adrDetail, setAdrDetail] = useState<string>(item.address_detail);
  ...
  const addAdrMutation = useMutation({
    mutationFn: addressApi.updateAdr,
    onSuccess: (data) => {
  ...
  const HandleAddAdr = () => {
    if (
      ...
      String(selectedProvince) === "Chọn Tỉnh, TP" ||
      ...
      addAdrMutation.mutate({
        province_id: String(selectedProvince),
  ...
  return (
    <div className="w-screen h-full">
      ...
      <div className="fixed bottom-[0%] w-full flex flex-col bg-white h-[10%] items-center justify-center">
        ...
        >
          <p className="text-sm text-white font-normal">Thay đổi</p>
        </div>
      </div>
    </div>
  );
